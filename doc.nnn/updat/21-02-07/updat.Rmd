...
Figures 1a-d show *case* counts per million for several Washington and non-Washington locations. Figures 1a-b (the top row) shows smoothed data (see details [below](#techdetails)); Figures 1c-d (the bottom row) overlay raw data onto the smoothed. The figures use data from Johns Hopkins Center for Systems Science and Engineering (JHU), described [below](#jhu). 

The Washington locations are the entire state, the Seattle area where I live, and the adjacent counties to the north and south (Snohomish and Pierce, resp.). The non-Washington locations are Ann Arbor, Boston, San Diego, and Washington DC. When comparing the Washington and non-Washington graphs, please note the difference in y-scale: the current Washington rates (1000-2000 per million) are similar to the rates in Ann Arbor and Washington DC, and slightly below the rates in Boston and San Diego.
...
The smoothed and raw data for Washington (Figures 1a,c) suggest that cases have hit their peak and are heading down. Simple trend analysis (described [below](#techdetails)) cautions that the apparent decline in Pierce (south of Seattle) may be spurious. The graphs for non-Washington locations (Figures 1b,d) suggest that cases everywhere except Ann Arbor have hit their peaks and are heading down, while Ann Arbor is flattening. Trend analysis concurs.

Figures 2a-d show *deaths* per million for the same locations. When comparing the Washington and non-Washington graphs, again please note the difference in y-scale: the current Washington rates (10-30 per million) are similar to the non-Washington rates.
...
The smoothed Washington data (Figure 2a) shows three waves. The second peak was thankfully lower than the first; the third wave is already higher than the first in all areas except Seattle (King County). The graphs are heading down now, but the raw data (Figure 2c) remains quite variable and trend analysis (described [below](#techdetails)) suggests the trends are weak. The smoothed non-Washington data (Figure 2b) shows early peaks in most location, followed by a long trough, followed by a second wave starting in November. Rates are declining everywhere except Washington DC. The raw data (Figure 2d) and trend analysis suggest the decline in Boston may be real, but the trends elsewhere are ill-defined.
...
Thankfully, cases seem to be heading down now.
...
3. The trend analysis mentioned above computes a linear regression (using R's `lm`) over the most recent four weeks of data and reports the computed slope and the p-value for the slope. In essence, this compares the trend to the null hypothesis that the true counts are constant and the observed points are randomly selected from a normal distribution. After looking at trend results across the entire time series, I determined that p-values below 0.1 indicate convincing trends; this cutoff is arbitrary, of course.
...
The previous two data release (versions 21-01-24 and 21-01-31) had additional problems: they were completely missing data at the end and had anomalously high counts at the beginning. My theory is that this was a "Y2.1K" problem; data that belonged in 2021 was instead reported with 2020 dates. I implemented a correction based on this theory and the results looked reasonable. Whatever the cause, DOH has fixed the problem in the current release (21-02-07).
