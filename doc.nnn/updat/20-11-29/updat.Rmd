...
Cases are rapidly increasing everywhere. In Washington state (Figure 1a), case counts are at their highest ever and continuing to rise. What jumps out from the non-Washington data (Figure 1b) are the high early peaks in Boston and DC, followed by smaller peaks in most locations, and rapidly increasing counts now. 
...
4. The geographic granularity in the underlyong data is *state* or *county*. I refer to locations by city names reasoning that readers are more likely to know "Seattle" or "Ann Arbor" than "King" or "Washtenaw".
...
The weekly downloads lag behind the daily feed causing data for the last few weeks to be incomplete. I correct for this undercount by extrapolating data from the preceeding six weeks using a linear model (R's `lm`). A quick looks suggests the extrapolation is presently overshooting by approximately 25% ([below](#extra) Figures 5a-b).
