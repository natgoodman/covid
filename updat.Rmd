...
date: "`r format(as.Date(vsn,format='%y-%m-%d'),'%B %d, %Y')`"
output:
  html_document:
...
The next graphs show the Washington results broken down by age. Figures 3a-d (the top row) are *cases*; Figures 4a-d (the bottom row) are *deaths*. 
The *cases* data is split into 20-year age ranges starting with 0-19, with a final group for 80+. The *deaths* graphs aggregate 0-59 into a single group, since the death rate in these ages is near 0.
...
Early on, the pandemic struck older age groups most heavily both in terms of *cases* and *deaths*. Over time, *cases* spread into all age groups, even the young. One notable difference in *cases* across locations is that in the latest wave, the oldest groups are doing better in Seattle than the surrounding counties, at least in relative terms. The *deaths* data in Seattle and Snohomish shows waves with declining peaks, but in Pierce, the peaks seem to be steady. The *deaths* data for Snohomish and Pierce are more variable; I suspect this reflects delays in reporting deaths to DOH.
...
4. The geographic granularity is *state* or *county*. I refer to locations by city names since readers are more likely to know "Seattle" or "Ann Arbor" rather than "King" or "Washtenaw".
...
The weekly downloads lag behind the daily feed, meaning that data for the last few weeks is incomplete. I correct for this undercount by extrapolating data from the preceeding six weeks using a linear model (R's `lm`).

The weekly DOH download reports data by age group: 20-year ranges starting with 0-19, with a final group for 80+.

The DOH download includes data on hospital admissions (*admits*) in addition to *cases* and *deaths*, though I don't show this data here.
